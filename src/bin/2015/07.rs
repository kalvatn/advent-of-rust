...
impl FromStr for Op {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        ["NOT", "AND", "OR", "LSHIFT", "RSHIFT"].iter().for_each(|opstr| {
            if s.contains(opstr) {
                op = match opstr {
                    &"NOT" => Not,
                    &"AND" => And,
...
impl FromStr for Instruction {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        fn get_input_args(op: &Op, s: &str) -> (Either<u16, String>, Either<u16, String>) {
            ...
            let a2 = if args.len() > 1 { args[1].to_string() } else { "".to_string() };
            let a3 = if args.len() > 2 { args[2].to_string() } else { "".to_string() };
            let args: Vec<Either<u16, String>> = vec![a1, a2, a3].iter().map(|a| parse_arg(a.trim())).collect();
        ...
        return Ok(Instruction { op, left, right, output });
    }
}
...
fn get_wire(instructions: &HashMap<String, Instruction>, results: &mut HashMap<String, u16>, wire: &Either<u16, String>) -> u16 {
    match wire {
        Either::Right(key) => {
...
fn part_two(input: &str) -> u16 {
    ...
    instructions.insert("b".to_string(), Instruction { op : Assign, left: Either::Left(part_one), right : Either::Left(0), output : "b".to_string()});
    return get_wire(&instructions, &mut results, &Either::Right("a".to_string()));
}
...
mod test {
    ...
    #[test]
    fn test_parse_input() {
        assert_eq!(parse_input("456 -> y"), vec![Instruction {
            op: Op::Assign,
            left: Either::Left(456),
            right: Either::Right("".to_string()),
            output: "y".to_string(),
        }]);
        assert_eq!(parse_input("NOT 456 -> y"), vec![Instruction {
            op: Op::Not,
            left: Either::Left(456),
            right: Either::Right("".to_string()),
            output: "y".to_string(),
...
NOT a -> h
NOT y -> i"), vec![
            Instruction { op: Assign, left: Either::Left(123), right: Either::Right("".to_string()), output: "a".to_string() },
            Instruction { op: Assign, left: Either::Left(456), right: Either::Right("".to_string()), output: "y".to_string() },
            Instruction { op: And, left: Either::Right("a".to_string()), right: Either::Right("y".to_string()), output: "d".to_string() },
            Instruction { op: Or, left: Either::Right("a".to_string()), right: Either::Right("y".to_string()), output: "e".to_string() },
            Instruction { op: Lshift, left: Either::Right("a".to_string()), right: Either::Left(2), output: "f".to_string() },
            Instruction { op: Rshift, left: Either::Right("y".to_string()), right: Either::Left(2), output: "g".to_string() },
            Instruction { op: Not, left: Either::Right("a".to_string()), right: Either::Right("".to_string()), output: "h".to_string() },
            Instruction { op: Not, left: Either::Right("y".to_string()), right: Either::Right("".to_string()), output: "i".to_string() },
        ]);
