...
struct WeightedGraph<T> where T: std::hash::Hash {
    edges: HashMap<T, HashMap<T, usize>>
}

#[derive(Debug, Eq, PartialEq, Clone, Hash)]
struct Path<T> where T: std::clone::Clone {
    path: Vec<T>,
    distance: usize,
}

#[derive(Eq, PartialEq, Clone, Hash)]
struct Node<T> where T: std::clone::Clone {
    value: T,
    distance: usize,
...
impl<T: std::cmp::Eq + std::hash::Hash + std::clone::Clone + Debug> WeightedGraph<T> {
    pub fn new() -> Self {
        WeightedGraph { edges: HashMap::new() }
    }

    pub fn connect(&mut self, source: T, target: T, weight: usize) {
        self.edges.entry(source.clone()).or_insert(HashMap::new()).entry(target.clone()).or_insert(weight);
        self.edges.entry(target.clone()).or_insert(HashMap::new()).entry(source.clone()).or_insert(weight);
    }
    ...
    pub fn find_best_path_visit_all(&self, start: T, shortest: bool, the_best: usize) -> Option<Path<T>> {
        ...
        queue.push_back(Node { value: start, distance: 0, parent: None });
        ...
        while !queue.is_empty() {
            ...
            let path = self.construct_path(current.clone(), Path { path: vec![], distance: 0 });
            ...
            self.edges.get(&current.value).unwrap().iter().for_each(|(k, v)| {
                let node = Node { value: k.clone(), distance: *v, parent: Option::from(Box::from(current.clone())) };
...
impl FromStr for Route {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        lazy_static! {
            static ref RE: Regex =
                Regex::new(r"^(\w+) to (\w+) = (\d+)$").unwrap();
        }
        let loc = RE.captures(s)
            .map(|cap| Route {
                a: Location { name: cap[1].to_string() },
                b: Location { name: cap[2].to_string() },
...
fn parse_input(input: &str) -> Vec<Route> {
    return input.lines().map(|line| Route::from_str(&line).unwrap()).collect();
}

fn part_one(input: &str) -> usize {
    let routes = parse_input(input);
    let mut graph: WeightedGraph<String> = WeightedGraph::new();
    routes.iter().for_each(|route| graph.connect(route.a.name.to_string(), route.b.name.to_string(), route.length));

    let mut best: Path<String> = Path { path: vec![], distance: usize::max_value() };
...
fn part_two(input: &str) -> usize {
    ...
    let mut best: Path<String> = Path { path: vec![], distance: usize::min_value() };
    for x in graph.edges.keys() {
...
Dublin to Belfast = 141";
    ...
    fn test_route_from_str() {
        assert_eq!(Route::from_str("London to Dublin = 464").unwrap(), Route {
            a: Location { name: "London".to_string() },
            b: Location { name: "Dublin".to_string() },
            length: 464,
        })
    }
    ...
    fn test_parse_input() {
        assert_eq!(parse_input(TEST_INPUT), vec![
            ...
            Route {
                a: Location { name: "London".to_string() },
                b: Location { name: "Belfast".to_string() },
                length: 518,
            },
            Route {
                a: Location { name: "Dublin".to_string() },
                b: Location { name: "Belfast".to_string() },
                length: 141,
            },
        ])
